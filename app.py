...
def show_movies():
    ...
    if sortMode == 'likes':
        movies = list(db.movies.find(
            {'trashed': False}, {}).sort('likes', -1))
    elif sortMode == 'viewers':
        movies = list(db.movies.find(
            {'trashed': False}, {}).sort('viewers', -1))
    elif sortMode == 'date':
        movies = list(db.movies.find(
            {'trashed': False}, {}).sort([('open_year', -1), ('open_month', -1), ('open_day', -1)]))
    else:
...
def show_trash():
    ...
    if sortMode == 'likes':
        movies = list(db.movies.find(
            {'trashed': True}, {}).sort('likes', -1))
    elif sortMode == 'viewers':
        movies = list(db.movies.find(
            {'trashed': True}, {}).sort('viewers', -1))
    elif sortMode == 'date':
        movies = list(db.movies.find(
            {'trashed': True}, {}).sort([('open_year', -1), ('open_month', -1), ('open_day', -1)]))
    else:
        return jsonify({'result': 'failure'})
